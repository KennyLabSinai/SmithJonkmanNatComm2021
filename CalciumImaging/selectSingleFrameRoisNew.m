% written by Mike
 % Pass this function a max-minus-min map. Allows you to
% manually select ROIs based on maximum values for each pixel in data.
function roi = selectSingleFrameRoisNew(singleFrame, viddir)
%     singleFrame = log(cast(singleFrame,'double'));
    sz = size(singleFrame);
    roiList = cell(0);
    Min = min(min(singleFrame)); Max = max(max(singleFrame));
    currFig = figure;
    H = imshow(singleFrame, [Min Max]); title('Select contrast'); 
    while(true)
    ...
    while(true)
        mb = msgbox('Zoom to desired area, then hit enter');
        zoom
        pause
        [xList,yList] = getpts;
    ...
    roi = cat(1,roiList{:});
    print(currFig, [viddir, 'ROIs'], '-dpng');
end
