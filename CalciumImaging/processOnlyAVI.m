...
function processOnlyAVI(varargin)
...
for n = 1:nFiles
	aviFile(n).fileName = fileName{n};
    v = VideoReader(fileName{n});
    aviFile(n).nFrames = v.NUMBEROFFRAMES;
end
...
for kFile = 1:numel(aviFile)
    vidstats = struct();
    curr_nFrames = aviFile(kFile).nFrames; % get number of frames in current file
    fprintf('Loading %d frames from  %s \n', curr_nFrames, fileName{kFile}); 
    data = loadAvi(aviFile(kFile).fileName); % load Tif file
    data = im2uint16(data);
    ...
    for f=1:MAX_FRAMES_PER_STEP:curr_nFrames
        ...
        if n_vid == 1 % if it is the first subvideo
            ...
            saveVidFile(curr_slice, fileName_char, n_vid,1);
    ...
    [dir,nm] = fileparts(aviFile(kFile).fileName);
    if isempty(dir)
        dir = pwd;
    end
...
if nargin
	...
	[fname,fdir] = uigetfile('/d1/studies/*.avi','MultiSelect','on');
	cd(fdir)
	switch class(fname)
		case 'char'
