classdef (CaseInsensitiveProperties = true) RegionOfInterest < hgsetget
	...
	properties
		...
		TDTOverlay							%  Background Image used in the showAsOverlay() method
	...
	methods % SHOW/DISPLAY
        ...
        function bigfig = showSimple(obj, overlayInput, contrastRange, coordinateShift)
        
            overlayInput = imtranslate(overlayInput,[coordinateShift(1), coordinateShift(2)]);

            bigfig = figure;
            imshow(overlayInput, [contrastRange(1) contrastRange(2)]);
            movegui('west')
            hold on
            for rId = 1:numel(obj)
                if obj(rId).isChI
                    plot(obj(rId).boundaryTrace.x, obj(rId).boundaryTrace.y,'g');
                elseif obj(rId).isPVI
                    plot(obj(rId).boundaryTrace.x, obj(rId).boundaryTrace.y,'b');
                else
                    plot(obj(rId).boundaryTrace.x, obj(rId).boundaryTrace.y,'r');
                end
                text(obj(rId).Centroid(1)+6, obj(rId).Centroid(2)-6,num2str(rId),'Color','g');
            end
            hold off
        end
        
        function bigfig = showSimpleWithTraces(obj, overlayInput, contrastRange, coordinateShift)
            fig = showSimple(obj, overlayInput, contrastRange, coordinateShift);
            figure
            movegui('east')
            plotActiveRegionsSimple(.0467*(1:length(obj(1).Trace))', rescale([obj.Trace])', zeros(size([obj.Trace]))');
        end
    ...
    methods % added functionality by Mike
        
        function varargout = showUnicolor(obj)
			%TODO: can make compatible for HG2 with: if verLessThan('matlab','8.4.0')
			global H
...
% 				end
         ...
         % % % % % % % % % % % % % % % % % % %         
        % Michael Romano 2/24/2016
        % Plots outline of multiple sets of ROIs 
        % % % % % % % % % % % % % % % % % % % %
        function varargout = findIntersections(obj, obj2, overlayInput)            
            global H
            obj = reduceSuperRegions(obj, 5, 10);
            obj2 = reduceSuperRegions(obj2, 5, 10);
            numObj = numel(obj);
            uidFirst = [obj.UID];
            uidLast = [obj2.UID];
            if nargin < 2
            ...
            H.im.ButtonDownFcn = @(src,evnt)roiCompareFcn(obj,src,evnt);
            
            if nargin > 2
                H.im.AlphaData = overlayInput;
                H.im.CData = repmat(ones(size(overlayInput)),1,1,3);
            end
            for kObj = 1:numel(obj)
                % get number of pixels for the first roi
                nPix = numel(obj(kObj).PixelIdxList);
                
                if any(uidLast == obj(kObj).UID)
                    currColor = 'blue';
                else
                    currColor = 'red';
                end
        ...
        function showAsOverlayOutline(obj, overlayInput, varargin)
			global H
			% DISPLAY/COMPARE
			sz = obj(1).FrameSize;
			...
			H.bg = image(zeros([sz 3] ,'uint8'),...
				'Parent', H.ax);
			H.bg.ButtonDownFcn = @(src,evnt)roiClickFcnTimeTrace(obj,overlayInput,src,evnt);
			% 		set(obj,'hBg',hbg);
			if isnumeric(overlayInput)
				% IMAGE
				if ismatrix(overlayInput)
					H.bg.AlphaData = imcomplement(overlayInput);
                    H.bg.AlphaData = overlayInput;
                    H.bg.CData = imcomplement(overlayInput);
					drawnow
					return
				else
					framePeriod = .05;%TODO: info structure
        ...
        function roiCompareFcn(obj, src, evnt)
                global H
                
                persistent selectedRoi
                ...
                R = RegionOfInterest.getRoi(obj, clickPoint);
                
                if ~isempty(selectedRoi)
                    disp(['connect roi ' num2str(R.UID) ' to ' num2str(selectedRoi.UID)]);
                    linkRoi(obj, R, selectedRoi);
%                     delete(H.line( find(obj == obj(R.UID) ) ) );
                    selectedRoi = [];
                else
                    selectedRoi = R;
                end
        ...
        function obj = linkRoi(obj,R1, R2)
            % obj MUST BE ORDERED BY idx
            idx1 = R1.UID;
            idx2 = R2.UID;
            obj(idx1).correspondingRoi = idx2;
            obj(idx2).correspondingRoi = idx1;
        end
        
        function obj = combineCorrespondingRoi(obj)
            deadRois = [];
            for j=1:numel(obj)
                currRoi = obj(j);
                if ~isempty(currRoi.correspondingRoi)
                    corrIndex = find([obj.UID] == currRoi.correspondingRoi);
                    newRoi = RegionOfInterest.concat(currRoi, obj(corrIndex));
                    obj(j) = newRoi;
                    deadRois = [deadRois; corrIndex];
                end
            end
            obj(deadRois) = [];
        end
        
        function obj = chiToFSI(obj)
            for r=1:numel(obj)
                chi = obj(r).isChI;
                obj(r).isPVI = chi;
                obj(r).isChI = false;
	...
	methods % COMBINATION/SUPBORDINATION METHODS
        ...
        function [roiOutput, indicesRemoved] = removeDoubleClicks(obj)
            roiOutput = [];
            indicesRemoved = [];
            for r=1:numel(obj)
                hasOverlap = false;
                for rr=1:numel(roiOutput)
                    if fractionalOverlap(obj(r), roiOutput(rr)) > .5
                        hasOverlap = true;
                        indicesRemoved = [indicesRemoved; r];
                        break
                    end
                end
                if ~hasOverlap
                    roiOutput = cat(2,roiOutput, obj(r));
                end
            end
        end
...
% 					obj(kRoi).HaloPixIdx = haloPixIdx;
        ...
        % Low-pass butterworth filters ROI trace.
        ...
        function obj = setChIIndices(obj, indices)
            for roi=1:numel(obj)
                if any(roi == indices)
                    obj(roi).isChI = true;
                end
        ...
        function obj = setPVIIndices(obj, indices)
            for roi=1:numel(obj)
                if any(roi == indices)
                    obj(roi).isPVI = true;
                end
		...
		function vidOverlayUpdate(~,src,~)
			try
				...
				if k > N		% Finished
					...
					if isnumeric(overlayInput)
						h.bg.AlphaData = overlayInput(:,:,k);
                        h.bg.CData = imcomplement(overlayInput(:,:,k));
					else
	...
	methods(Static)
        function R = getRoi(roiObj, clickPoint)
            R = [];
            for k=1:numel(roiObj)
                if prod(any(bsxfun(@eq,roiObj(k).PixelSubScripts, clickPoint)))
                    R = roiObj(k);
                end
            end
        end

        function R = concat(rFixed, rMoved)
            rFixed.Trace = cat(1, rFixed.Trace, rMoved.Trace);
            R = rFixed;
        end
    
        function roiPairObj = pairRoisFromCorresponding(roiList,foutname)
            roiPairObj = cell(0);
            for j=1:numel(roiList)
                currRoi = roiList(j);
                % We want the pre-infusion roi to be the first input to
                % Roipair constructor, and post-infusion to be the second
                % Should only look at Rois that have corresponding
                % pre-infusion rois
                if ~currRoi.isPostInfusion
                    % if the roi has a corresponding roi
                    correspondingRoiUID = currRoi.correspondingRoi;
                    if ~isempty(correspondingRoiUID)
                        % find the corresponding roi with the UID
                        corrIndex = find([roiList.UID] == correspondingRoiUID);
                        % if nothing matches, continue
                        if isempty(corrIndex)
                            continue
                        elseif length(corrIndex) > 1
                            corrIndex = corrIndex(1);
                        end
                        postRoi = roiList(corrIndex);
                        
                        % if this roi is a post-infusion roi
                        if postRoi.isPostInfusion
                            roiPairObj{end+1} = RoiPairs(currRoi, roiList(corrIndex));
                        end
                    end
                end
            end
            roiPairObj = cat(1,roiPairObj{:});
            roiPairObj = resetIDs(roiPairObj);
            if nargin > 1
                save(foutname,'roiPairObj');
            end
        end
